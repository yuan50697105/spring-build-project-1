-- auto Generated on 2021-04-29
DROP TABLE IF EXISTS t_personal_report_check_items;
CREATE TABLE t_personal_report_check_items(
	id BIGINT (15) NOT NULL AUTO_INCREMENT COMMENT 'id',
	create_id BIGINT (15) DEFAULT -1 COMMENT 'createId',
	update_id BIGINT (15) DEFAULT -1 COMMENT 'updateId',
	create_user VARCHAR (50) DEFAULT '' COMMENT 'createUser',
	update_user VARCHAR (50) DEFAULT '' COMMENT 'updateUser',
	create_date DATETIME DEFAULT '1000-01-01 00:00:00' COMMENT 'createDate',
	update_date DATETIME DEFAULT '1000-01-01 00:00:00' COMMENT 'updateDate',
	patient_id BIGINT (15) DEFAULT -1 COMMENT 'patientId',
	fee_item_id BIGINT (15) DEFAULT -1 COMMENT 'feeItemId',
	check_item_id BIGINT (15) DEFAULT -1 COMMENT 'checkItemId',
	check_item_name VARCHAR (50) DEFAULT '' COMMENT 'checkItemName',
	result_type VARCHAR (50) DEFAULT '' COMMENT 'resultType',
	lower_limit VARCHAR (50) DEFAULT '' COMMENT 'lowerLimit',
	upper_limit VARCHAR (50) DEFAULT '' COMMENT 'upperLimit',
	detection_value VARCHAR (50) DEFAULT '' COMMENT 'detectionValue',
	conclusion VARCHAR (50) DEFAULT '' COMMENT 'conclusion',
	PRIMARY KEY (id)
)ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT 't_personal_report_check_items';
